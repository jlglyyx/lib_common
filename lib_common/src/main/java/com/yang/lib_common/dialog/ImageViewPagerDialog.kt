...
class ImageViewPagerDialog(
    ...
    override fun onCreate() {
        super.onCreate()
        mBinding.tvCount.text = "1/${data.size}"
        initViewPager()
        initToolbar()
    }
    ...
    private fun initToolbar() {
        ...
        if (showCollection) {
            mBinding.ivCollection.visibility = View.VISIBLE
            mBinding.ivCollection.clicks().subscribe {
                imageViewPagerDialogCallBack?.onViewClickListener(this)
                mBinding.ivCollection.setImageResource(R.drawable.iv_collection_click)
            }
        }
    ...
    private fun initViewPager() {
        mBinding.viewPager.registerOnPageChangeCallback(object : ViewPager2.OnPageChangeCallback() {
            override fun onPageSelected(mPosition: Int) {
                ...
                mBinding.tvCount.text = "${mPosition + 1}/${data.size}"
                position = mPosition
            }
        })
        mBinding.viewPager.adapter = object : RecyclerView.Adapter<RecyclerView.ViewHolder>() {
    ...
    override fun onDismiss() {
        imageViewPagerDialogCallBack?.getPosition(mBinding.viewPager.currentItem)
        GSYVideoManager.releaseAllVideos()
        super.onDismiss()
    }
